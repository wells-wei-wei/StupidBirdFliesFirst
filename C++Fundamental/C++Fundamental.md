...
- [C++基础知识](#c基础知识)
  ...
  - [函数模板](#函数模板)
    - [显式具体化](#显式具体化)
    - [一个函数名可以有几种函数情况](#一个函数名可以有几种函数情况)
    - [实例化](#实例化)
    - [重载解析](#重载解析)
    - [函数模板中的其他情况](#函数模板中的其他情况)
  - [单独编译](#单独编译)
    - [头文件](#头文件)
  - [不同的内存持续性](#不同的内存持续性)
  - [不同的作用域和链接](#不同的作用域和链接)
  ...
  - [类](#类)
    ...
    - [引入this](#引入this)
    - [引入const](#引入const)
    - [类的成员函数和变量之间的顺序关系](#类的成员函数和变量之间的顺序关系)
    - [类外部的函数](#类外部的函数)
    - [返回对象本身的函数](#返回对象本身的函数)
    - [类的相关非成员函数](#类的相关非成员函数)
    - [构造函数](#构造函数)
    ...
    - [重载成员函数](#重载成员函数)
    - [可变数据成员](#可变数据成员)
    - [返回this的成员函数](#返回this的成员函数)
    - [从const成员函数返回*this](#从const成员函数返回this)
    - [类中的基于const的函数重载](#类中的基于const的函数重载)
    - [类的静态成员](#类的静态成员)
    - [类的静态对象](#类的静态对象)
    - [类继承](#类继承)
      - [基类和派生类的关系](#基类和派生类的关系)
...
&emsp;&emsp;const的变量在C++中的编译时就已经确定了，这时会存放在编译器的符号表中。计算时编译器直接从表中取值，省去了访问内存的时间。所以假设有程序从const变量中取得了地址，并从地址中进行修改的话，确实能够把const给修改了，如果从这个地址去读的话是会发生的改变的：
```
const int i = 0; 
int *j = (int *) &i; 
*j = 1; 
...
// 这时会显示0和1
```
但是因为读取原本的const变量还是从符号表中去读，所以用这个值做任何计算都没有影响。

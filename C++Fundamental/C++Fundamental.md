 ...
 * @LastEditTime: 2020-07-13 17:44:28
...
- [33. 类](#33-类)
  ...
  - [33.13. 拷贝控制操作](#3313-拷贝控制操作)
  - [33.14. 拷贝构造函数](#3314-拷贝构造函数)
  - [33.15. 合成拷贝构造函数](#3315-合成拷贝构造函数)
  - [33.16. 拷贝赋值运算符](#3316-拷贝赋值运算符)
  - [33.17. 合成拷贝赋值运算符](#3317-合成拷贝赋值运算符)
  - [33.18. 析构函数](#3318-析构函数)
  - [33.19. 合成析构函数](#3319-合成析构函数)
  - [33.20. 定义的规则](#3320-定义的规则)
  - [33.21. 重载运算符](#3321-重载运算符)
  - [33.22. 聚合类](#3322-聚合类)
  - [33.23. 字面值常量类](#3323-字面值常量类)
  - [33.24. 类的访问控制与封装](#3324-类的访问控制与封装)
  - [33.25. 友元](#3325-友元)
  - [33.26. 定义类型成员](#3326-定义类型成员)
  - [33.27. 令成员作为内联函数](#3327-令成员作为内联函数)
  - [33.28. 重载成员函数](#3328-重载成员函数)
  - [33.29. 可变数据成员](#3329-可变数据成员)
  - [33.30. 返回this的成员函数](#3330-返回this的成员函数)
  - [33.31. 从const成员函数返回*this](#3331-从const成员函数返回this)
  - [33.32. 类中的基于const的函数重载](#3332-类中的基于const的函数重载)
  - [33.33. 类的静态成员](#3333-类的静态成员)
  - [33.34. 类的静态对象](#3334-类的静态对象)
  - [33.35. 类继承](#3335-类继承)
  - [33.36. 基类和派生类的关系](#3336-基类和派生类的关系)
  - [33.37. 公有继承](#3337-公有继承)
    - [33.37.1. 多态公有继承](#33371-多态公有继承)
  - [33.38. 虚函数](#3338-虚函数)
    - [33.38.1. 访问控制](#33381-访问控制)
    - [33.38.2. 抽象基类（纯虚函数）](#33382-抽象基类纯虚函数)
    - [33.38.3. 重载、重定义和重写](#33383-重载重定义和重写)
  - [33.39. 私有继承](#3339-私有继承)
  - [33.40. 保护继承](#3340-保护继承)
  - [33.41. 多重继承](#3341-多重继承)
    - [33.41.1. 虚基类](#33411-虚基类)
  - [33.42. 类模板](#3342-类模板)
...
&emsp;&emsp;但是也不是随便加const都能重载，函数的参数总共就是三种情况：按值传递，引用和指针。对于函数值传递的情况，因为参数传递是通过复制实参创建一个临时变量传递进函数的，函数内只能改变临时变量，但无法改变实参。则这个时候无论加不加const对实参不会产生任何影响。但是在引用或指针传递函数调用中，因为传进去的是一个引用或指针，这样函数内部可以改变引用或指针所指向的变量，这时const 才是实实在在地保护了实参所指向的变量。因为在编译阶段编译器对调用函数的选择是根据实参进行的，所以，只有引用传递和指针传递可以用是否加const来重载。
...
&emsp;&emsp;这里还需要解释一下初始化和赋值的概念，初始化是要创建一个新的对象，并且其初值来源于另一个已存在的对象；赋值操作则是在两个已经存在的对象间进行的。对于那些普通的内置类型来说这种区别基本能忽略不计，因为就像上面说的，就算参数列表中没有它，也可以在构造函数体之前初始化它（问题只是有没有初值能不能用而已），所以对于内置类型无论是用这种参数列表还是在构造函数里写个赋值语句都是一样的。但是有一部分例外，只能使用参数列表：
```
class ConstRef{
public:
    ConstRef(int i);
...
拷贝构造函数一定要用const的引用，因为假如是实参，那就是通过按值传递机制传递的，在可以传递对象之前，编译器需要安排创建该对象的副本。因此，编译器为了处理复制构造函数的这条调用语句，需要调用复制构造函数来创建实参的副本。但是，由于是按值传递，第二次调用同样需要创建实参的副本，因此还得调用复制构造函数，就这样持续不休。最终得到的是对复制构造函数的无穷调用。（其实就是创建副本也是需要调用复制构造函数的）
...
&emsp;&emsp;注意如果一个类有多个构造函数，那么析构函数必须能与每个构造函数都能兼容，所以构造函数里使用new的时候要么都带中括号，要么都不带。
...
## 33.21. 重载运算符
&emsp;&emsp;上面已经介绍了拷贝运算符和写法，很多c++的运算符都可以用这种方式重载。其中也有一些限制：
1. 重载后的运算符必须有一个操作数是用户定义的，这样是为了防止用户为内置类型重载运算符；
2. 使用运算符时不能违反运算符原本的句法规则，例如不能把求模运算符重载成一个操作数（像&）
3. 不能创建新的运算符
4. 一些运算符不能被重载：
   - sizeof
   - . 成员运算符
...
- 定义在private之后的成员只能被类的成员函数访问，不能被外部访问，这里体现封装。但是还有其他方法可以让其他类或函数访问private，也就是友元

## 33.25. 友元
&emsp;&emsp;只有一个类把一个函数作为它的友元，那么就可以让函数去访问它的非公有成员，只要在类中声明它并加上friend即可：
```
class Box
{
   double width;
...
```
友元的声明必须出现在类定义的内部，但是出现的位置不限，需要注意的是它确实不是类的成员。友元在类定义中声明仅仅是说明它有类的访问权限，并不是普通意义上的那种声明，所以真要使用的话还得在类外再声明一次，或者就像上面那样直接定义，注意，在外部的声明或者定义是必须有的，否则友元函数在哪都不能用。

&emsp;&emsp;另外友元函数也可以是其他类的成员函数：
```
class A;    //当用到友元成员函数时，需注意友元声明与友元定义之间的互相依赖。这是类A的声明
class B
{
    public:
        void set_show(int x, A &a);             //该函数是类A的友元函数，这里不能定义，只能先声明
};
...
```
这里由于类之间互相使用所以必须注意定义和声明的顺序。

&emsp;&emsp;也可以把其他的类定义为友元类：
```
class A
{
    public:
...
```
只要指定了友元类，这个类的成员函数就可以访问此类的所有成员。但是友元不具备传递性，如果class C也有自己的友元，那么它并不能访问class A的成员。

&emsp;&emsp;友元在很多情况下都用于对运算符的重载，因为实际上一个a*b的简单算式，细究起来应该是a调用了一个\*的operator，用在类中，比如一个自定义的Time类乘一个内置的double类，这时Time在前就是Time调用了一个重载的运算符\*，但是如果double在前那就不行了，那就会直接调用c++内置的那种\*，就会报错。这时为了应对这种情况，友元函数就是必须的了，在外部重载一个\*，然后调用Time的类成员，这样才能达到目的。

## 33.26. 定义类型成员
&emsp;&emsp;使用typedef在类中定义一种类型：
```
class MyClass
{
public:
...
```
这就是在类中给long起了一个叫INDEX的别名，因为是public的，所以在类外也能使用：
```
MyClass::INDEX s=100;
```
这种定义一般是为了隐藏一些实现，比如不想让别人知道我用的数据类型是long，但是还需要外部输入这种类型，就用这种方式换个名字。如果是定义在private中，则只能在类中使用，而且必须先定义再使用。

## 33.27. 令成员作为内联函数
&emsp;&emsp;一部分规模较小的函数适合被用作内联函数。如果是在类内定义的成员函数则自动就是内联函数，外部的如果想让它成为内联函数则必须用inline。

## 33.28. 重载成员函数
&emsp;&emsp;跟普通的重载函数类似，返回值可以不同，但是特征标（参数个数或者类型）必须不同。

## 33.29. 可变数据成员
&emsp;&emsp;mutable在类中同样是可用的：
```
class Screen  
{  
public:  
...
```
理论上加上const的函数就是无法修改成员的（传递const的this指针），但是由于access_ctr是mutable的，所以只有它是可以修改的。这种主要作用就是创造例外。

## 33.30. 返回this的成员函数
&emsp;&emsp;前面已经说过了this的作用其中之一就是返回对象本身：
```
Sd& Sd::combing(){
    return *this;
}
```
这种时候记得前面一定要加&，这样才能返回对象本身，否则返回的就是对象的一个副本。

## 33.31. 从const成员函数返回*this
&emsp;&emsp;this本身是个const指针，也就是说可以改变指向的具体值，但是不能改变指向的地址。假如成员函数定义了一个const的返回类型：
```
...
```
这里返回值定义成const其实是必须的，因为函数后加了const以后此时的this就是既不能改变地址又不能改变指向的值。而要返回它自然也应该是这样的值，即const Screen。但是这样的话返回的值以后就再也没法改了，就不能再执行那种改变自身成员的成员函数了：
```
Screen myScreen;
myScreen.display(cout).set('*');
```
当然如果不想执行以上这种命令序列的话其实也没啥问题，但是想执行的时候就会出问题了，因为myScreen.display(cout)返回的值是没法改的。

## 33.32. 类中的基于const的函数重载
...
## 33.34. 类的静态对象
&emsp;&emsp;类在创建对象的时候也可以使用static创建静态的对象，这种对象不会在作用域结束的时候调用析构函数，会贯穿整个程序的生命周期。

## 33.35. 类继承
&emsp;&emsp;类的继承是提供方法来扩展或者修改原来的基类，新生成的类叫派生类：
```
class B:public A
{
    ......
...
```
这里的public表示公有派，A是一个公有基类。这种情况下。基类的公有成员将成为派生类的公有成员，基类的私有成员也将成为派生类的一部分，但是不能直接使用，只能通过基类的公有和保护方法使用。这里的派生类还需要自己的构造函数并添加自己的函数和成员。

&emsp;&emsp;派生类中不能直接访问基类的私有成员，所以在继承之后，派生类的构造函数也必须使用基类的构造函数。在创建派生类对象的时候程序也会首先创建基类的对象，c++使用成员初始化列表完成这项工作：
```
R::R(int i,string s):Rbase(i,s){}
```
这里的Rbase(i,s)就是基类的构造函数，派生类的构造函数其实就是把基类的构造函数放在初始化列表里，以保证能够在生成派生类对象之前生成基类对象。由于在创建派生类对象之前创建基类对象是必须的，所以就算构造函数中没写上基类的构造函数也会被编译器自动生成（使用默认的构造函数）。

&emsp;&emsp;无论如何，派生类肯定是先生成基类对象，通过派生类的构造函数将参数传递给基类构造函数然后生成，然后才能管派生类自己新的数据成员的初始化。

## 33.36. 基类和派生类的关系
&emsp;&emsp;派生类可以直接使用基类的公有函数，而基类的指针可以在不显示类型转换的情况下指向派生类（但是不能调用派生类的方法），基类引用也可以不转换就引用派生类对象。另外不可以将基类的对象和地址赋给派生类的引用和指针。

## 33.37. 公有继承
&emsp;&emsp;最普通的一种继承，以上的种种情况都是公有继承之后的产物。
### 33.37.1. 多态公有继承
&emsp;&emsp;一般希望在基类中的函数在派生类中有一定的变化，这就是多态。这种时候有两种方法实现这种多态：
- 在派生类中重新定义基类方法
- 使用虚函数

## 33.38. 虚函数
如果基类函数在派生类中被修改，再想调用基类的函数的话就要加基类名和作用域解析符号。虚函数就是在基类中使用virual关键字声明出虚方法，然后在派生类中重写方法即可。在可以重新定义的同时还需要虚函数的原因是：为了实现一种方法可以在派生类和基类中的行为不同，于是在派生类中重新定义基类函数的方法（比如基类有方法A， 但又在派生类中重新定义了方法A， 它们名字一样，但行为不一样），然后通过各自的对象调用各自的方法。但有时候我们会用一个基类类型的引用或者指针来指向派生类的对象，这时如果你使用引用来调用方法，会调用基类的方法A ，还是调用在派生类中重新定义的方法A 呢？答案是：基类的方法A。很简单，因为引用（reference）的类型是基类的。为了能让程序能够根据基类类型的引用（reference）所引用的对象来选择方法，所以定义了虚方法。因此，虚方法是“重新定义基类方法”的扩展，为的是让引用或者指针更加智能地选择对象所对应的方法。
```
class Base1 {
public:
...
&emsp;&emsp;另外虚函数也是可以内联的，但是在虚函数表现多态的时候是不可以，因为内联是在编译器建议编译器内联，而虚函数的多态性在运行期，编译器无法知道运行期调用哪个代码，因此虚函数表现为多态性时（运行期）不可以内联。这样一来唯一可以内联的情况就是编译器知道所调用的对象是哪个类，这只有在编译器具有实际对象而不是对象的指针或引用时才会发生。

&emsp;&emsp;特别注意，构造函数不能是虚函数。当定义一个对象的时候，首先会分配对象内存空间，然后调用构造函数来初始化对象。虚指针变量是在构造函数中进行初始化的。又因为执行虚函数需要通过虚指针来调用。如果可以定义构造函数为虚函数，那么就会陷入先有鸡还是先有蛋的循环讨论中。

&emsp;&emsp;另外构造函数中也不要去调用虚函数，当然这么写一般不会报错，因为一个构造函数，先按照对象的内存布局进行初始化，因为虚表指针是放在顶部的，先初始化虚表指针（也就是说虚指针位于一个对象实例内存中最前面的位置，主要为了保证性能），指向虚表（虚表在编译期就生成），之后按照声明顺序初始化成员变量。最后调用构造函数{ }中的代码，所以说在调用构造函数之前虚表和虚指针就已经有了，这么写是没问题的。但是调用的结果一般是不符合预期的。因为在构造派生类时，先使用基类的构造函数，这时如果在基类的构造函数中有一个虚函数，则因为此时派生类还没被创建出来，只可能使用基类的虚函数而不是我们希望的那样使用派生类的构造函数。但是假如基类构造中没用虚函数，只在派生类构造中使用还是可以的。

&emsp;&emsp;但是基类的析构函数则一定要是虚函数。因为一般我们使用虚函数时都是使用一个基类指针去指向派生类对象，假如不把基类的析构函数设置成虚函数，那么就会导致只调用基类的析构函数而不去调用派生类的构造函数，这会导致内存泄漏。
...
综合来说，公有继承的特点是基类的公有成员和保护成员作为派生类的成员时，它们都保持原有的状态，而基类的私有成员仍然是私有的，不能被这个派生类的子类所访问。私有继承的特点是基类的公有成员和保护成员都作为派生类的私有成员，并且不能被这个派生类的子类所访问。保护继承的特点是基类的所有公有成员和保护成员都成为派生类的保护成员，并且只能被它的派生类成员函数或友元访问，基类的私有成员仍然是私有的。基类成员对派生类都是：共有和保护的成员是可见的，私有的的成员是不可见的。基类成员对派生类的对象来说：要看基类的成员在派生类中变成了什么类型的成员。如：私有继承时，基类的共有成员和私有成员都变成了派生类中的私有成员，因此对于派生类中的对象来说基类的共有成员和私有成员就是不可见的。
...
## 33.43. 内联的类模板
&emsp;&emsp;很多时候我们能遇见这样的宏函数：
```
#define  MAX( x, y )  ( ((x) > (y)) ? (x) : (y) )
```
这种函数的目的跟类模板其实差不多。但是我们都知道应该尽量不使用#define（就像之前说过的用const和enum代替#define），所以这里可以用inline来表示模板，达到与宏函数相同的作用：
```
template <typename T>
...
## 33.50. typename
C++在模板编程中可能会用到typename，这是一种描述符，用来确认当前这个模板参数是一个类型而不是变量。例如：
```c++
template <class T>
void foo() {
    T::iterator * iter;
...
```
这种时候我们可以理解是模板参数T中有一个嵌套类叫iterator，在foo中新建一个iterator类的指针；但同时也可以理解是模板类T中有一个iterator的变量，然后在foo中与iter相乘。这种时候在理解上一定会出现偏差，所以可以在T::iterator之前加上typename，用来确定T::iterator一定是一个类名而不是变量。

...
&emsp;&emsp;另外构造函数中也不要去调用虚函数，当然这么写一般不会报错，因为一个构造函数，先按照对象的内存布局进行初始化，因为虚表指针是放在顶部的，先初始化虚表指针，指向虚表（虚表在编译期就生成），之后按照声明顺序初始化成员变量。最后调用构造函数{ }中的代码，所以说这么写是没问题的。但是调用的结果一般是不符合预期的。因为在构造派生类时，先使用基类的构造函数，这时如果在基类的构造函数中有一个虚函数，则因为此时派生类还没被创建出来，只可能使用基类的虚函数而不是我们希望的那样使用派生类的构造函数。但是假如基类构造中没用虚函数，只在派生类构造中使用还是可以的。

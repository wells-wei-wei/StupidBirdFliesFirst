...
- [2. 进程并发](#2-进程并发)
  ...
  - [2.3. 进程同步](#23-进程同步)
    ...
    - [2.3.2. 临界区](#232-临界区)
    - [2.3.3. 同步锁](#233-同步锁)
      - [2.3.3.1. 互斥锁](#2331-互斥锁)
      - [2.3.3.2. 条件锁](#2332-条件锁)
      - [2.3.3.3. 读写锁](#2333-读写锁)
      - [2.3.3.4. 自旋锁](#2334-自旋锁)
      - [2.3.3.5. 文件锁](#2335-文件锁)
    - [2.3.4. 管程](#234-管程)
    - [2.3.5. 事件](#235-事件)
  - [2.4. 进程通信](#24-进程通信)
    - [2.4.1. 管道](#241-管道)
    ...
    - [2.4.6. 套接字](#246-套接字)
  - [2.5. 进程调度](#25-进程调度)
    - [2.5.1. 批处理系统](#251-批处理系统)
      - [2.5.1.1. 先来先服务 first-come first-serverd（FCFS）](#2511-先来先服务-first-come-first-serverdfcfs)
      - [2.5.1.2. 短作业优先 shortest job first（SJF）](#2512-短作业优先-shortest-job-firstsjf)
      - [2.5.1.3. 最短剩余时间优先 shortest remaining time next（SRTN）](#2513-最短剩余时间优先-shortest-remaining-time-nextsrtn)
    - [2.5.2. 交互式系统](#252-交互式系统)
      - [2.5.2.1. 时间片轮转](#2521-时间片轮转)
      - [2.5.2.2. 优先级调度](#2522-优先级调度)
      - [2.5.2.3. 多级反馈队列](#2523-多级反馈队列)
    - [2.5.3. 实时系统](#253-实时系统)
...
#### 2.5.1.1. 先来先服务 first-come first-serverd（FCFS）
非抢占式的调度算法，按照请求的顺序进行调度。

有利于长作业，但不利于短作业，因为短作业必须一直等待前面的长作业执行完毕才能执行，而长作业又需要执行很长时间，造成了短作业等待时间过长。

#### 2.5.1.2. 短作业优先 shortest job first（SJF）
非抢占式的调度算法，按估计运行时间最短的顺序进行调度。

长作业有可能会饿死，处于一直等待短作业执行完毕的状态。因为如果一直有短作业到来，那么长作业永远得不到调度。

#### 2.5.1.3. 最短剩余时间优先 shortest remaining time next（SRTN）

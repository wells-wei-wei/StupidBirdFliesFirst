...
# 2. Linux的IO多路复用
## 2.1. select
select的原理其实跟非阻塞IO（NIO）是一样的，但是如果用NIO写个服务的话，那就必须是在while里一直循环遍历所有的socket，直到找到发生事件的那个。但是这种自己遍历的方法需要从用户态切换到内核态，有10000个socket的话就需要核心态切换10000次，效率低。select是把所有客户端连接传递给内核，内核遍历返回可读可写的，减少了系统调用的次数，也就是可以把10000个客户端的连接传给操作系统，返回所有连接中有事件发生的连接，程序再自己读取事件(程序自己读取IO)，获取状态原来10000个连接需要10000次系统调用，select一次系统调用即可获取状态。
```c++
#include <sys/types.h> 

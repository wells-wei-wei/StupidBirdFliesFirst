...
- [1. 分类](#1-分类)
  - [1.1. 从数据结构分类](#11-从数据结构分类)
  - [1.2. 物理存储角度](#12-物理存储角度)
    - [1.2.1. 聚集索引（聚簇索引）](#121-聚集索引聚簇索引)
    - [1.2.2. 非聚集索引](#122-非聚集索引)
  - [1.3. 逻辑角度](#13-逻辑角度)
    - [1.3.1. 主键索引](#131-主键索引)
    - [1.3.2. 普通索引或单列索引](#132-普通索引或单列索引)
    - [1.3.3. 复合索引](#133-复合索引)
    - [1.3.4. 唯一索引](#134-唯一索引)
    - [1.3.5. 全文索引](#135-全文索引)
    - [1.3.6. 总结](#136-总结)
- [2. B树](#2-b树)
  - [2.1. 平衡二叉树](#21-平衡二叉树)
  - [2.2. B树](#22-b树)
  - [2.3. B树的插入操作](#23-b树的插入操作)
  - [2.4. B树的删除操作](#24-b树的删除操作)
- [3. B+树](#3-b树)
  - [3.1. B+树的插入操作](#31-b树的插入操作)
  - [3.2. B+树的删除操作](#32-b树的删除操作)
- [4. 不同数据库的不同选择](#4-不同数据库的不同选择)
...
## 1.3. 逻辑角度
### 1.3.1. 主键索引
主键索引（主索引）是唯一索引的特定类型。表中创建主键时自动创建的索引 。一个表只能建立一个主索引。不可为空。
...
### 1.3.5. 全文索引
MyISAM 存储引擎对全文索引的支持有很多的限制，例如表级别锁对性能的影响、数据文件的崩溃、崩溃后的恢复等，这使得 MyISAM 的全文索引对于很多的应用场景并不适合。所以，多数情况下的建议是使用别的解决方案，例如 Sphinx、Lucene 等等第三方的插件，亦或是使用 InnoDB 存储引擎的全文索引。
